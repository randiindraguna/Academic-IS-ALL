...
<?php 
    ...
    session_destroy();
    $db_penguji = $akses->getDataDosenPenguji1byIdJadwal($id_jadwal);
    foreach ($db_penguji as $key) {
        $niy_dosen_penguji = $key['niy'];
        $nama_penguji = $key['nama_dosen'];
    }
    $db_penguji = $akses->getDataDosenPenguji2byIdJadwal($id_jadwal);
    foreach ($db_penguji as $key) {
        $niy_dosen_penguji2 = $key['niy'];
        $nama_penguji2 = $key['nama_dosen'];
    }
?>
</head>
<body class="">
        ...
        <div class="col-12 ">
            <form id="fform" action="../control/edit_pendadaran.php" method="post">
            <div class="row">
            ...
            <div class="col-6 mt-2">               
                    ...
                    <select id="inputState" class="form-control" id="penguji" name="penguji2">
                            <option selected value="<?=$niy_dosen_penguji2?>"><?=$nama_penguji2?></option>
                            <?php foreach ($akses->getAllDosenKecualiDuaDosen($niy_dosen_penguji,$niy_dosen_penguji2) as $key ) {?>                            
